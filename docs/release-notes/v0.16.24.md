...
- **Landebahn-Erkennung bei einem Divert nutzt jetzt die echten
  ...
  ASA-ACARS die Landebahn immer gegen die Navdata des GEPLANTEN Ziels.
  ...
  ASA-ACARS fiel auf die grobe eingebaute OurAirports-CSV zurück. Folge:
  ...
  Ab jetzt bestimmt ASA-ACARS aus der Touchdown-Koordinate den
...
- **Vorab-Laden der Divert-Navdata schon im Anflug.** Erkennt ASA-ACARS
...
- **Bestmögliche Daten beim PIREP-Filing — auf BEIDEN Oberflächen.** Falls
  ...
  ASA-ACARS zunächst das provisorische OurAirports-Ergebnis, stößt aber
...
- **Divert runway detection now uses the real Navigraph navdata of the
  ACTUAL landing airport.** Until now ASA-ACARS always correlated the
  ...
  from the touchdown — the Navigraph lookup missed and ASA-ACARS fell back
  ...
  and with it the rollout / runway-utilization score. Now ASA-ACARS derives
...
- **Approach-phase pre-fetch of the divert airport's navdata.** When
  ASA-ACARS detects, during a stable descending approach, that the nearest
...
- **Finalize-with-best at PIREP filing — on BOTH surfaces.** If the cache
  wasn't warm at touchdown (a late divert), ASA-ACARS stamps the provisional
