...
> **Upload:** ab v0.5.23 nach erfolgreichem PIREP-File automatisch an asa-acars-live (`POST /api/flight-logs/upload`)
...
|---|---|---|
| `block`-Snapshot | `asa-acars/<va>/<pilot>/block` MQTT | JSONL hat `block_fuel_kg`/`planned_burn_kg` etc. nur in der finalen `flight_ended`-Indirektion (= via `flight.stats`); kein dediziertes Block-Event |
| `takeoff`-Snapshot | `asa-acars/<va>/<pilot>/takeoff` MQTT | Wie oben — kein dedizierter Takeoff-Snapshot im JSONL |
| Touchdown-Events | `asa-acars/<va>/<pilot>/touchdown` MQTT | Nur `landing_scored` (= finales Aggregat). Multi-Touchdown-Pattern (Touch-and-Go-Training) nicht differenzierbar |
| PIREP-Body | `asa-acars/<va>/<pilot>/pirep` MQTT | JSONL hat `flight_ended { outcome }` aber NICHT was tatsaechlich gefilet wurde (Distanz/Zeit/Notes/Custom-Fields) |
...
BlockSnapshot {
    ...
    /* alle Felder aus asa-acars-mqtt::BlockPayload */
},
TakeoffSnapshot {
    timestamp: DateTime<Utc>,
    /* alle Felder aus asa-acars-mqtt::TakeoffPayload */
},
TouchdownEvent {
    timestamp: DateTime<Utc>,
    vs_fpm: i32,
...
**Server-Speicherort:** `/var/lib/asa-acars-recorder/flight-logs/<va>/<pilot>/<pirep_id>.jsonl.gz`
...
> **Telemetrie ab v0.5.23:** TouchdownPayload schickt `simulator` + `vs_estimate_xp_fpm` + `vs_estimate_msfs_fpm` + `vs_source` + `gear_force_peak_n` + `estimate_window_ms` + `estimate_sample_count` an asa-acars-live damit der VA-Owner Algorithmen-Vergleiche im Diagnostics-Tab + LandingAnalysis-Modal sieht.
...
**Bekannte Edge-Cases:**
- *2026-05-07 MYNN→MBGT:* X-Plane Lua-Tool sagte 273 fpm, ASA-ACARS sagte -394 fpm (~44% zu hoch). Ursache: Time-Tier-Estimator zu starr bei niedriger RREF-Rate. Behoben in v0.5.13 (Umstellung auf Lua-30-Sample-adaptive).
- *2026-05-06 DAL93 EDDB→KJFK:* echter -300 fpm, ASA-ACARS scorte +35 fpm (smooth) weil Streamer 5 s nach Touchdown wachte und Buffer dann nur Rollout-Samples mit V/S≈0 enthielt. Behoben in v0.4.4 (Sampler-side Edge-Detection bei 50 Hz).
