# ASA-ACARS-Integrity-Gate ↔ VPS-Recorder — Integration-Test-Playbook

**Spec:** v0.13.0  
**Repos:**
- VPS Recorder: `asa-acars-live` (Hetzner, live.kant.ovh)
- Gate-Modul: `asa-acars-integrity-gate` (private, MANFahrer-GF)
- phpVMS-Host: `atlanticstarairways.com`
...
```
       ...
       │   PHPVMS HOST (atlanticstarairways.com)     │     │
       │  - PirepFiled-Event                     │     │
       │  - ASA-ACARSIntegrityGate-Modul         │─────┘
       │  - /api/asa-acars-gate/webhook/decision │
       │  - /admin/asa-acars-gate (Review-UI)    │
       └────────────────────────────────────────┘
```
...
Auf `atlanticstarairways.com` (oder Testumgebung):
...
git clone git@github.com:MANFahrer-GF/asa-acars-integrity-gate.git ASA-ACARSIntegrityGate
cd ASA-ACARSIntegrityGate
composer install --no-dev
cd ../..

# Migrations laufen lassen
php artisan migrate --path=modules/ASA-ACARSIntegrityGate/database/migrations

# Modul aktivieren (Konvention)
php artisan module:enable ASA-ACARSIntegrityGate

# Config publishen
php artisan vendor:publish --tag=asa-acars-integrity-gate-config
...
**Auf VPS (`/etc/asa-acars-recorder/env` oder via systemd-unit):**
```
AEROACARS_GATE_BASE_URL=https://atlanticstarairways.com
AEROACARS_GATE_SHARED_SECRET=3f8e9a2b4c5d6e7f8901a2b3c4d5e6f7890a1b2c3d4e5f60718293a4b5c6d7e8
```
...
# VPS
sudo systemctl restart asa-acars-recorder
```
...
Auf phpVMS-Host:
```bash
php artisan asa-acars:integrity-auto-repair-check
# Erwartete Ausgabe wenn drift: "Auto-repaired X rank(s) where ..."
# Wenn schon clean: "All ranks clean."
```

Im phpVMS-Admin: `/admin/asa-acars-gate` öffnen → "Gate-Safety-Status" muss **grün** sein.
...
Auf der VPS:
```bash
cd /opt/asa-acars-live/recorder
# Source der Service-ENV damit AEROACARS_GATE_SHARED_SECRET da ist
export $(systemctl show asa-acars-recorder -p Environment | tr ' ' '\n' | grep AEROACARS_GATE_SHARED_SECRET | sed 's/Environment=//')
# Oder manuell exportieren

npm run test-gate-integration https://atlanticstarairways.com
```
...
| Test 1 → unreachable | DNS oder Firewall | `curl -v https://atlanticstarairways.com` |
...
1. **Pilot**: Sim starten, ASA-ACARS-Client öffnen, Bid wählen (z.B. GSG123 EDDF→LOWW), Flug fliegen normal.
2. **PIREP filen** via ASA-ACARS-Client.
3. **Recorder-Log beobachten** (VPS):
   ```
   journalctl -u asa-acars-recorder -f | grep integrity
   ```
...
4. **phpVMS-Log beobachten** (phpVMS-Host):
   ```
   tail -f storage/logs/laravel.log | grep ASA-ACARS-Gate
   ```
   Erwartet:
   - `ASA-ACARS-Gate per-PIREP auto-repair` (nur wenn drift)
   - `ASA-ACARS-Gate verdict applied (in-flow) ... verdict=clean`
...
3. **phpVMS-Log:**
   ```
   ASA-ACARS-Gate verdict applied (in-flow) ... verdict=untrusted
   ```
4. **phpVMS PIREP:**
   - State: `PENDING` (NICHT ACCEPTED!)
   - Status: `INT_HOLD_UNTR`
5. **Gate-Review-UI** (`/admin/asa-acars-gate/review`):
...
1. **In ASA-ACARS-Live Webapp** (`https://live.kant.ovh/admin/#/review`):
...
3. **phpVMS-Log:**
   ```
   ASA-ACARS-Gate webhook: decision applied
...
```bash
# Auf VPS
sudo systemctl stop asa-acars-recorder
```

1. PIREP filen via phpVMS-API
2. **phpVMS-Log:** `ASA-ACARS-Gate: VPS unreachable, PENDING-Hold`
3. **phpVMS PIREP:** State `PENDING`, Status `INT_HOLD_TIMEOUT`
4. Recorder restart: `sudo systemctl start asa-acars-recorder`
5. Warten max. 5 Min (gate AsyncVerdictRetryQueue)
6. **phpVMS-Log:** `ASA-ACARS-Gate verdict applied (off-flow)`
...
1. PIREP filen
2. **phpVMS-Log:** `ASA-ACARS-Gate webhook: hmac_invalid`
...
curl https://atlanticstarairways.com/admin/asa-acars-gate/status.json | jq
...
```php
$schedule->command('asa-acars:integrity-auto-repair-check')->dailyAt('04:00');
$schedule->command('asa-acars:integrity-retry-queue-drain')->everyFiveMinutes();
$schedule->command('asa-acars:integrity-recorder-callback-drain')->everyTwoMinutes();
```
...
| Alle PIREPs sofort ACCEPTED | `auto_approve_acars=true` UND Auto-Repair down | `asa-acars:integrity-auto-repair-check` manuell, dann Discord-Alert investigieren |
| `INT_HOLD_PENDING` Stuck | not_ready-Loop, TD-Event nie angekommen | Pilot-Client-Log prüfen, Session-last_seen schauen |
| `INT_REJ_REPAIR_FAIL` | AutoRepair-Failure (DB-Lock, Plugin-Conflict) | Laravel-Log, manuelle DB-Korrektur, then `php artisan asa-acars:integrity-claim-cleanup` |
| 409 `claim_incomplete` von Webhook | Worker hängt oder gecrashed | `ps aux \| grep queue:work`, ggf `asa-acars:integrity-claim-cleanup --decision-id=...` |
| Recorder-Review-Tab inkonsistent zu phpVMS | Callback-Queue abandoned | `asa-acars:integrity-recorder-callback-drain --limit=200` manuell laufen |
