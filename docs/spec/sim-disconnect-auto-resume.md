...
2. Wenn der Sim wieder Daten liefert, resumed ASA-ACARS automatisch.
...
| ASA-ACARS laeuft weiter | 30 Sekunden Heartbeat-Takt | phpVMS-PIREP bleibt aktiv, obwohl der Sim gerade keine Positionsdaten liefert |
| Sim-Crash, ASA-ACARS bleibt offen | praktisch unbegrenzt | Heartbeat laeuft weiter; sobald der Sim wieder Daten liefert, soll Auto-Resume greifen |
| MSFS/X-Plane-Neustart, ASA-ACARS bleibt offen | praktisch unbegrenzt | gleicher Fall wie Sim-Crash; Drift wird geloggt, nicht geblockt |
...
Wichtig: Die 6 Stunden sind kein Client-Timer, sondern das effektive phpVMS-/Server-Fenster nach dem letzten Lebenszeichen. Waehrend ASA-ACARS noch laeuft, verhindert der 30-Sekunden-Heartbeat genau diesen Ablauf.

### Was passiert bei Sim-Crash?

Wenn nur der Simulator abstuerzt oder neu gestartet wird, ASA-ACARS aber offen bleibt:

- nach 30 Sekunden ohne Snapshot wird der Flug pausiert
- ASA-ACARS sendet weiter Heartbeats an phpVMS
...
`asa-acars-live` muss bei eingehenden Events/Positionen mit `pirep_id` zuerst versuchen:
...
ASA-ACARS soll einen laufenden Flug nach Sim-Disconnect, Sim-Crash, MSFS/X-Plane-Neustart oder Rechner-Reboot sauber weiterfuehren, solange derselbe phpVMS-PIREP (`pirep_id`) gemeint ist.
...
- `asa-acars-live/recorder/src/mqttSubscriber.ts`
- `asa-acars-live/recorder/src/db.ts`
...
Ziel: X-Plane soll einen aktiven Pause-Zustand an ASA-ACARS melden koennen, damit die Pausezeit auch dort sauber gezaehlt wird.
...
- ASA-ACARS pausiert den Flug automatisch, wenn der Simulator keine Daten mehr liefert.
...
- ASA-ACARS warnt, wenn nach Recovery ein anderes Flugzeug geladen ist.
```
...
- ASA-ACARS automatically pauses the flight when the simulator stops sending data.
...
- ASA-ACARS warns if a different aircraft is loaded after recovery.
```
...
- `ensureSession(...)` in `asa-acars-live/recorder/src/mqttSubscriber.ts` priorisiert aktuell `latest` + `matchesFlightContext(...)` + `RESUME_WINDOW_MS`.
