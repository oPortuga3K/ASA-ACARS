...
Heute hat ASA-ACARS 17 interne `FlightPhase`-Varianten (`Preflight, Boarding, Pushback, TaxiOut, TakeoffRoll, Takeoff, Climb, Cruise, Holding, Descent, Approach, Final, Landing, TaxiIn, BlocksOn, Arrived, PirepSubmitted`). Drei davon spiegeln Sonderfaelle aber NICHT explizit wider:
...
### Vergleich Stratos v2.14.0 (NUR-Referenz, NICHT ASA-ACARS-Wire-Contract)

> ⚠️ **WICHTIG — diese Sektion ist Stratos-Referenz, nicht ASA-ACARS-Wirklichkeit.**
...
>
> **Verbindlich für ASA-ACARS sind ausschließlich die 18 Phase-Strings aus [`v0.9.0-telemetry-contract.md`](v0.9.0-telemetry-contract.md) Sektion 1.3.** Diese Spec fügt davon GENAU 3 neue Werte hinzu: `REJECTED_TAKE_OFF`, `GO_AROUND`, `DEBOARDING`. Die unten zitierten Stratos-Werte werden NICHT übernommen — sie dienen ausschließlich als Architektur-Inspiration für die Trigger-Werte (45 kt, 8°/70 %, Engines-off etc.).
>
> Konkrete Mapping-Tabelle Stratos→ASA-ACARS am Ende dieser Sektion.
...
```
// STRATOS-INTERN (NICHT ASA-ACARS!):
...
**Stratos→ASA-ACARS-Wire-Contract-Mapping** (= falls jemand fragt „warum nicht 1:1"):

| Stratos-String | ASA-ACARS-String (kanonisch laut Contract Sektion 1.3) | Anmerkung |
|---|---|---|
| `UNKNOWN` | (nicht übernommen) | ASA-ACARS startet immer in `PREFLIGHT`, kein UNKNOWN-Zwischenzustand |
| `BOARDING` | `BOARDING` | identisch |
| `PUSH_BACK` | `PUSHBACK` | ASA-ACARS schreibt ohne Underscore — bestehend, nicht geändert |
| `TAXI` | `TAXI_OUT` | ASA-ACARS unterscheidet TAXI_OUT vs TAXI_IN, Stratos nicht |
| `TAKE_OFF` | `TAKEOFF_ROLL` + `TAKEOFF` | ASA-ACARS hat zwei Sub-Phasen (Rollen + Lift-off), Stratos eine |
| `REJECTED_TAKE_OFF` | **`REJECTED_TAKE_OFF`** (NEU v0.9.0) | exakt übernommen |
| `CLIMB`/`CRUISE`/`DESCENT`/`APPROACH`/`FINAL` | identisch | — |
| `LANDED` | `LANDING` | ASA-ACARS-Begriff bestehend |
| `GO_AROUND` | **`GO_AROUND`** (NEU v0.9.0) | exakt übernommen |
| `TAXI_IN`/`ARRIVED` | identisch | — |
| (Stratos hat nicht) | `BLOCKS_ON` | ASA-ACARS-spezifisch, bestehend |
| `DEBOARDING` | **`DEBOARDING`** (NEU v0.9.0) | exakt übernommen |

**Verbindlich ist NUR die rechte Spalte.** Die Stratos-Strings werden im ASA-ACARS-Code nirgendwo geschrieben, gelesen oder verarbeitet — sie sind reine Vergleichsreferenz.
...
### Wire-Format — MQTT-`phase_label()` (`asa-acars-mqtt/src/lib.rs`)
...
Recorder (`asa-acars-live/recorder/src/db.ts`):
...
Keine Schema-Aenderung. Server-Seite (`asa-acars-live/recorder`) speichert den String wie er kommt.
...
| -------------------- | --------------------------- | -------------------- |
| `REJECTED_TAKE_OFF`  | „ASA-ACARS — CALL ROUTE · RTO" | „Startabbruch"     |
...
| MQTT phase_label       | `client/src-tauri/crates/asa-acars-mqtt/src/lib.rs` (Zeile ~1281)                           | drei neue match-arms                                                                       |
...
| Webapp Types           | `asa-acars-live/webapp/src/types.ts`                                                        | `FlightPhaseString`-Union erweitern                                                       |
| Webapp Live-Map        | `asa-acars-live/webapp/src/components/LiveMap.tsx`, `PilotMarker.tsx`                       | Farb-Mapping, Tooltip-i18n                                                                |
| Webapp PIREP-Card      | `asa-acars-live/webapp/src/components/PirepCard.tsx`                                        | RTO/GA-Badges, Filter-Dropdown im Owner-Dashboard                                         |
| Webapp i18n            | `asa-acars-live/webapp/src/locales/{de,en,it}/common.json`                                  | drei neue Strings                                                                          |
| Recorder DB (optional) | `asa-acars-live/recorder/src/db.ts`                                                         | `ALTER TABLE pireps ADD COLUMN IF NOT EXISTS rto_count INTEGER DEFAULT 0` (idempotent)    |
| Recorder PIREP-Endpoint| `asa-acars-live/recorder/src/server.ts`                                                     | optional `rto_count`-Feld aus PIREP-Submission persistieren                               |
