...
ASA-ACARS hat in v0.7.15 bereits eine breite Fenix-Anbindung (Lichter,
...
- ASA-ACARS Pilot Client `v0.7.16` (Windows, normales Stable-Update)
...
Im ASA-ACARS Settings-Panel → Beta → **Fenix A32x Beta** anschalten.
...
| Schritt | Erwartung |
|---|---|
| Im ASA-ACARS-Inspector `L:S_OH_EXT_LT_LANDING_BOTH` adden | Wert sollte mit `_L` und `_R` synchron sein |
...
| Sim-Restart bei aktivem Flug | ASA-ACARS Auto-Resume (v0.7.15) greift, Profil wird neu detektiert |

---

## Verifikation im ASA-ACARS-Inspector
...
4. ASA-ACARS-Activity-Log-Ausschnitt rund um die Zeit
5. Optional: Screenshot vom ASA-ACARS-Inspector mit den LVAR-Werten
...
Wenn ein Wert nicht erkannt wird oder fehlt: ASA-ACARS bleibt im
