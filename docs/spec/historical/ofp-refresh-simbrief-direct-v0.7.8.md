...
> **Kern-Entscheidung Thomas (2026-05-11):** SimBrief-direct (Variante B aus dem Vorgaenger-Spec §11) wird umgesetzt. Begruendung: ASA-ACARS-internal, kein server-coordinated PAX-Studio-Deploy noetig, SimBrief ist ohnehin die Wahrheits-Quelle der OFP-Werte.
...
SimBrief speichert latest OFP fuer User X
       │
       ▼
[Pilot klickt "⟳ Aktualisieren" im ASA-ACARS-Bid-Tab]
       │
       ▼
ASA-ACARS liest SimBrief-Identifier aus Settings ← v0.7.8 NEU
...
SimBrief liefert latest OFP (XML mit dpt/arr/callsign/etc.)
       │
       ▼
ASA-ACARS Flight-Match-Verifikation:
...
### Was wir ASA-ACARS-seitig parsen muessen (v1.3-final)

**v1.3 QS-Entscheidung:** `static_id` kommt **NICHT** in den Parser, **NICHT** in `FlightStats`, **NICHT** in Settings. Begruendung (aus Navigraph-Forum-Thread): `static_id` ist fuer Systeme die OFPs selber **erzeugen** ueber die API. ASA-ACARS fetcht nur — wir brauchen keinen Slot-Pointer.
...
ASA-ACARS hat `airline_icao` UND `flight_number` als getrennte Felder in `ActiveFlight` — das ist die saubere Quelle. Wir konstruieren beide Seiten zur Vergleichs-Form:
...
/// "DLH100" → ("DLH", "100"), "100" → ("", "100"), "GSG100A" → ("ASA", "100A").
...
fn ofp_matches_active_flight(
    ...
    // 2. Callsign-Match: ASA-ACARS hat airline_icao + flight_number als
...
**Problem (QS Thomas 2026-05-11):** Real-Pilot-Beispiel von ASA:
...
Es ist nicht sicher ob phpVMS-`/api/user/bids` bei ASA-Pilot-Bids das `flight.callsign`-Feld bereits mit "4TK"/"CFG4TK" befuellt — oder ob es leer/anders ist (z.B. "CFG1504" weil phpVMS die Bid-Flight-Number copy-pasted hat).

**Pragmatischer Ansatz v1.5:** ASA-ACARS liest **beide** Quellen additiv:
...
Optional empirische Verifikation per Pilot-Test (`https://atlanticstarairways.com/api/user/bids` im Browser eingeloggt aufrufen, `bid.flight.callsign`-Feld inspizieren) — bestaetigt welches Feld bei ASA befuellt ist. Nicht-blockierend, da Code beide Felder additiv liest.
...
3. ASA-ACARS holt latest OFP direkt von SimBrief
...
2. Pilot klickt "Aktualisieren" im ASA-ACARS Bid-Tab (= fuer den aktiven kommerziellen Flug)
3. ASA-ACARS holt latest OFP — Mismatch (anderer dpt/arr/callsign)
...
3. ASA-ACARS faellt auf Pointer-Pfad → `bid_not_found`
...
1. ASA-ACARS versucht SimBrief-direct → Network-Error (oder HTTP 5xx)
...
- **v1.3 (2026-05-11):** Final-vor-Code nach Thomas-QS auf Navigraph "Fetching a User's Latest OFP Data"-Doku:
  - §3 SimBriefOfp-Parser: `static_id`-Feld komplett raus (war in v1.2 noch als Option enthalten). Begruendung Forum-Thread: `static_id` ist fuer Systeme die OFPs **erzeugen**, nicht fetchen. ASA-ACARS fetcht nur — wir brauchen keinen Slot-Pointer. Parser bekommt nur `request_id: String`.
...
- **v1.5 (2026-05-11):** Release-Blocker-Fix nach QS-Report Thomas (Real-Pilot-Case CFG1504 vs CFG4TK):
  ...
  - §6.1.4 Empirik-Hinweis: ASA-ACARS liest BEIDE Quellen additiv (Flight.callsign existiert bereits im Parser, Profile.callsign neu). Robust gegen jede phpVMS-VA-Konfiguration.
