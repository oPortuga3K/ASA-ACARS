# ASA-ACARS X-Plane Plugin

Native X-Plane plugin (XPLM SDK 4.3.0, C++17) that pairs with the
**ASA-ACARS** desktop ACARS client (v0.5.0+) to provide frame-perfect
telemetry — most importantly, frame-perfect **touchdown landing-rate
capture**.

| Field          | Value                                                   |
|----------------|---------------------------------------------------------|
| Plugin name    | `ASA-ACARS Premium`                                     |
| Signature      | `com.asa-acars.xplane.premium`                          |
...
The plugin is **strictly optional**. ASA-ACARS without it works exactly
...
1. **Cadence:** the ASA-ACARS streamer ticks every 5 s, so by the time
...
The ASA-ACARS desktop installer copies the plugin automatically. If
you're installing manually:

1. Download the `ASA-ACARS-XPlane-Plugin-vX.Y.Z.zip` artifact from
   the GitHub Release page.
2. Extract it into your X-Plane plugins folder so you end up with:
   ```
   <X-Plane>/Resources/plugins/ASA-ACARS/
       64/
...
3. Restart X-Plane. Open `Plugins → Plugin Admin` — you should see
   "ASA-ACARS Premium" listed as enabled.
4. Open the ASA-ACARS desktop client → Settings → Debug. The
   "X-Plane Premium Plugin" panel should turn green within a few
   seconds.

If the panel doesn't turn green:

* Open `<X-Plane>/Log.txt` and grep for `[ASA-ACARS]` — every
  log line from the plugin is prefixed with that.
* Check that no other ASA-ACARS instance is running (port 52000
...
cmake --build build --config Release --target ASA-ACARS
# Output: build/ASA-ACARS/64/win.xpl
```
...
cmake --build build --target ASA-ACARS
# Output: build/ASA-ACARS/64/mac.xpl
```
...
cmake --build build --target ASA-ACARS
# Output: build/ASA-ACARS/64/lin.xpl
```
...
Drop the whole `build/ASA-ACARS/` folder into
...
The plugin source is licensed identically to ASA-ACARS itself — see
