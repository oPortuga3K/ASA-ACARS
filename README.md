...
## Für wen läuft ASA-ACARS? · Who can run ASA-ACARS?

🇩🇪 **Der Quellcode ist frei — die offiziellen Apps laufen nur für die German Sky Group.**

ASA-ACARS ist Open Source (MIT-Lizenz). Jede Virtual Airline darf den Code
...
[GitHub-Releases](https://github.com/MANFahrer-GF/ASA-ACARS/releases)) sind
dagegen fest auf die German Sky Group konfiguriert: Login, Live-Tracking und
PIREP-Submission funktionieren ausschließlich mit einem ASA-Pilotenaccount —
ein Account einer anderen VA wird abgewiesen. Wer ASA-ACARS für eine andere VA
...
ASA-ACARS is open source (MIT). Any virtual airline is welcome to clone, adapt
and build the code for its **own** phpVMS 7 instance.

The **officially released builds** (the installers in the
[GitHub releases](https://github.com/MANFahrer-GF/ASA-ACARS/releases)) are, by
contrast, hard-wired to the German Sky Group: login, live tracking and PIREP
submission only work with a ASA pilot account — accounts of other VAs are
rejected. To use ASA-ACARS for a different VA, build your own client from
...
Lade dir das Paket für deine Plattform aus dem [Latest Release](https://github.com/MANFahrer-GF/ASA-ACARS/releases/latest) herunter.

### Windows (10 / 11, x64)

1. `ASA-ACARS_<version>_x64-setup.exe` (NSIS-Installer) herunterladen und ausführen
2. SmartScreen-Warnung wegklicken: „Weitere Informationen" → „Trotzdem ausführen" — wir sind noch nicht code-signed
3. ASA-ACARS startet nach der Installation automatisch
4. Login mit deinem phpVMS-API-Key

### macOS (Apple Silicon — M1 / M2 / M3 / M4)

1. `ASA-ACARS_<version>_aarch64.dmg` herunterladen
2. DMG öffnen → ASA-ACARS-Icon in den Applications-Ordner ziehen
3. **Beim ersten Start:** Gatekeeper blockt die App, weil sie nicht über die Apple Notarization gegangen ist. Du hast zwei Wege:
   - **Per Rechtsklick:** Im Finder auf ASA-ACARS rechtsklicken → „Öffnen" → „Öffnen" im Dialog bestätigen. Danach merkt sich macOS die Erlaubnis und startet die App ab dann normal.
...
## Schultern, auf denen ASA-ACARS steht
...
Wer ein Study-Level-Flugzeug fliegt: einmal die Tabelle mit dem [DataRefTool](https://datareftool.com/) ausfüllen und über den [Issue-Tracker](https://github.com/MANFahrer-GF/ASA-ACARS/issues) einreichen — dann kann das Flugzeug ein passendes DataRef-Profil bekommen.

---

## Troubleshooting / Logs

Wenn ASA-ACARS sich komisch verhält und du dem Issue-Tracker etwas Substanzielles mitschicken willst — hier ist, was wo liegt.

### Wo ASA-ACARS Daten ablegt

Alle Dateien liegen unter dem Tauri-Standard-`app_data_dir` mit Bundle-ID `com.asa-acars.app`:

| Plattform | Vollständiger Pfad |
|---|---|
| **Windows** | `%APPDATA%\com.asa-acars.app\` <br>(typisch: `C:\Users\<dein-user>\AppData\Roaming\com.asa-acars.app\`) |
| **macOS** | `~/Library/Application Support/com.asa-acars.app/` |

In Windows kannst du den Ordner direkt mit `Win+R` → `%APPDATA%\com.asa-acars.app` öffnen. In macOS mit Finder → `Cmd+Shift+G` → den Pfad einfügen.
...
- **Windows:** ASA-ACARS aus einer PowerShell-Konsole starten: `& "C:\Program Files\ASA-ACARS\ASA-ACARS.exe"` — dann erscheinen die tracing-Zeilen im Terminal.
- **macOS:** Aus dem Terminal: `/Applications/ASA-ACARS.app/Contents/MacOS/ASA-ACARS`

Verbosity-Level steuern via `RUST_LOG`:

```bash
# Standardmodus (info)
RUST_LOG=info  ./ASA-ACARS

# Volles Debug für unseren Code, info für alles andere
RUST_LOG=info,asa-acars=debug  ./ASA-ACARS
```
...
3. Falls reproduzierbar: ein paar Zeilen tracing-Output mit `RUST_LOG=info,asa-acars=debug` aus dem Terminal-Run

Issues bitte über → [github.com/MANFahrer-GF/ASA-ACARS/issues](https://github.com/MANFahrer-GF/ASA-ACARS/issues)
