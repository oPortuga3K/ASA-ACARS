...
const READ_STORAGE_KEY = "asa-acars.readNewsIds";
...
export function useUnreadNewsCount(loggedIn: boolean): number {
  ...
  useEffect(() => {
    ...
    window.addEventListener("asa-acars:news-read-changed", onCustom);
    return () => {
      window.removeEventListener("storage", onStorage);
      window.removeEventListener("asa-acars:news-read-changed", onCustom);
    };
  }, []);
...
export function NewsPanel() {
  ...
  function markRead(id: number): void {
    setReadIds((prev) => {
      ...
      window.dispatchEvent(new Event("asa-acars:news-read-changed"));
      return next;
    });
  }
