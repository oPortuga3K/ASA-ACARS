...
// v0.12.12-dev: ASA-Wetter-Briefing-Seite. Login-basiert — wenn der Pilot
// in seinem Standard-Browser bei phpVMS eingeloggt ist, zieht die Seite
// den aktiven Bid automatisch.
const WEATHER_BRIEFING_URL = "https://atlanticstarairways.com/weatherbriefing";
...
export function CockpitView({
  ...
  // nur, wenn das ASA-ACARS-Fenster im Vordergrund UND der Cockpit-Tab
  // aktiv war (sonst drosselt die WebView die JS-Timer). Pilot-Befund:
  // der PIREP ging erst raus, nachdem der Pilot ASA-ACARS in den
  ...
  // v0.12.12-dev: Wetter-Briefing-Knopf öffnet die ASA-Briefing-Seite im
  ...
  const flightIdent = activeFlight
    ? activeFlight.airline_icao
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-zinc-950 relative w-full">
      ...
      <header className="h-16 flex-shrink-0 flex items-center justify-between px-8 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-20 w-full">
        <div className="flex items-center gap-6">
          <h2 className="text-xl font-bold text-white">Cockpit</h2>
          <div className="h-6 w-px bg-zinc-800"></div>
          {activeFlight ? (
            <div className="flex items-center gap-4 text-sm font-mono">
              <span className="text-sky-400 font-bold">{flightIdent}</span>
              <span className="text-zinc-500">|</span>
              <span className="text-zinc-300">{activeFlight.aircraft_name || activeFlight.aircraft_icao}</span>
            </div>
          ) : (
            <span className="text-sm font-mono text-zinc-500">Idle</span>
          )}
        </div>
      </header>
      ...
      <div className="flex-1 p-8 w-full">
        ...
        {activeFlight && !activeFlight.was_just_resumed && (
          <DivertBanner
