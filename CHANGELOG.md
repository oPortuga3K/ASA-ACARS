...
**Stream B — VA-Härtung: nur aktive ASA-Piloten.** Login, Live-Tracking und PIREP-Provisioning prüfen jetzt den Piloten-Status aus phpVMS. Nur ein **aktiver** Account kommt durch — wartende, abgelehnte, beurlaubte, gesperrte oder gelöschte Accounts werden mit einer status-spezifischen Meldung abgewiesen. Die Durchsetzung läuft client- **und** server-seitig (Recorder). Hintergrund: GitHub-Issue #16.
...
Webapp-Mirror (`asa-acars-live`) zieht die Stream-B-Recorder-Härtung nach.
...
Spec: `docs/spec/v0.12.0-runway-utilization-refinement.md`. Webapp-Mirror (`asa-acars-live`) zieht die identische Score-Logik nach.
...
### Neu — Approach-Stability-Card (analog zur asa-acars-live-Webapp)
...
- **Form-State wird in localStorage persistiert** (Key `asa-acars.vfr.<bid_id>`) — überlebt jeden React-Remount, Resize, oder kurzen App-Reload. Werte werden nach erfolgreichem Flug-Start oder explizitem Cancel gelöscht.
...
- `client_version` aus dem MQTT-Payload (= `CARGO_PKG_VERSION` des Pilot-Clients, schon seit v0.7.x mitgesendet) wird jetzt im Webapp-PIREP-Detail-Header als kleine Pill angezeigt — `📱 ASA-ACARS v0.11.0`. VA-Owner sieht auf einen Blick mit welcher Client-Version ein PIREP produziert wurde — wichtig wenn Anomalien diagnostiziert werden müssen. (`asa-acars-live/webapp/src/components/LandingAnalysis.tsx`)
...
- **Settings → Discord Rich Presence**: 3 Toggles
  ...
  - **Callsign anonymisieren** ("GSG3184" → "ASA-Flight", Route bleibt sichtbar)
...
- `large_image` = ASA-ACARS-Logo (Brand-Konsistenz)
...
Self-hosted Sentry-kompatible Fehler-Sammelstelle. ASA-ACARS (Client) + Recorder (VPS) + Webapp (Admin-UI) senden anonymisierte Crash- und Error-Events automatisch hin, sodass der VA-Owner Bugs sieht **bevor** Pilots im Discord klagen.
...
- **Self-hosted GlitchTip-Stack** auf der VPS: Docker-Compose (postgres + redis + web + worker), Caddy mit auto Let's-Encrypt-Cert, 4 Uptime-Monitore eingerichtet (Recorder + GlitchTip self + ASA-phpVMS + ASA-API)
...
- Beide Stellen (Pilot-Client Rust-Crate UND asa-acars-live Webapp) sind in Sync zu fixen — diese Version fixt den Pilot-Client; das Webapp-Repo bekommt einen separaten Patch
...
- ✅ Pilot muss nur ASA-ACARS-Update installieren + Schalter umlegen
...
Real-Pilot-Incident **AUA 323 LOWW→ESGG am 2026-05-11** (PIREP `J2VoaZmoD6LQGpMg`): MSFS friert im Descent ein, ACARS pausiert nach 30 s, Pilot bemerkt es erst nach manueller Landung am Boden in ESGG. Resultat: zwei Sessions im History-Tab, Block-Time-Drift, ASA-ACARS-Recorder hat eine 23-min Lücke nicht überbrückt.
...
- Initial-State zuverlässig: wenn ASA-ACARS connectet während MSFS schon pausiert ist, kommt sofort ein initialer Pause_EX1-Event
...
Server-Patches sind in `asa-acars-live` (commits `92b22c6` + `0ffceca`) gepusht. Auf `live.kant.ovh` deployen via `deploy-recorder.sh`.
...
- URL liegt **nur** in SQLite-DB des Recorders (`/var/lib/asa-acars-recorder/`)
...
Komplettes QS-Audit über beide Codebases (Pilot-Client + asa-acars-live + Cross-Cutting-Security). 3 parallele Auditor-Agenten haben **18 Punkte** identifiziert. Dieses Release pickt alle **Pilot-Client-relevanten** Punkte raus (= SSH/VPS-Changes wurden bewusst ausgeklammert für einen späteren Release).

### Critical-Fix

**A1 — Discord-Webhook-Token nicht mehr hardcoded** (Audit C1)
- `discord.rs:27` hatte den ASA-Discord-Webhook-Token im Klartext drin. Repo ist public auf github.com/MANFahrer-GF/ASA-ACARS → das Token war effektiv öffentlich.
...
| # | Was |
|---|---|
| B1 | Tauri-Updater-Private-Key aus `client/asa-acars-updater.key` nach `~/.asa-acars-keys/` verschoben (war in `.gitignore`, aber 1× `git add -f` = catastrophic). GitHub-Actions-Secrets nicht betroffen. |
...
| D4 | Audit-Reports (`pilot-client-audit.md`, `asa-acars-live-audit.md`, `security-audit.md`, `MASTER-AUDIT-REPORT.md`) im Repo committet |
...
- v0.7.10: DAL804 → phpVMS `-407 fpm` (SimVar) / ASA-ACARS-UI `-364 fpm` (50-Hz-Edge) → Pilot verwirrt
- v0.7.11: DAL804 → phpVMS `-364 fpm` / ASA-ACARS-UI `-364 fpm` → konsistent
...
Reine UI-Erweiterung, keine neue Datenerhebung. Adressiert wiederkehrende Pilot-Beschwerden vom Typ *"Volanta zeigt mir 232 fpm aber ASA-ACARS scort 357 — wer hat Recht?"* — beide Werte sind physikalisch korrekt, sie messen unterschiedliche Sachen. Die neue Sektion erklaert das transparent.
...
1. **Aufklaerungs-Block** (cyan-Akzent): "Welche Sinkrate ist die richtige?" — erklaert dass Volanta/Cockpit-VSI Mittel ueber 0.5-1.5 s zeigen, ASA-ACARS aber den Cascade-Wert direkt am Aufsetz-Moment scort (FAR 25.473 Engineering-Standard)
...
### Design-Konsistenz mit ASA-ACARS-Look (§4.5)

Spec verbietet "Stein im ASA-ACARS-Design". Implementiert mit:
...
- **31 Vitest-Tests** in `SinkrateForensik.test.tsx` (neu) — Bucket-Math, Trend-Detection, Coaching-Selector, Score-Basis-Cascade, Tone-Bands, Trace-Filter, ASA-218-End-to-End
...
2. **W5 Bid weg nach Prefile** — phpVMS-7 entfernt den Bid sofort wenn ASA-ACARS prefiled. Damit ist der gesamte phpVMS-Pointer-Pfad fuer OFP-Refresh **tot** sobald der Pilot in Boarding ist. Cockpit-Button auch.
...
Auto-Updater zieht v0.7.7 automatisch fuer alle bestehenden v0.7.6-Pilot-Clients. Pilot kann nach Update einmalig SimBrief-Username/User-ID in Settings eintragen (oder ohne SimBrief-direct weiter Pointer-Pfad nutzen). Ohne Settings-Eintrag funktioniert ASA-ACARS exakt wie v0.7.6.
...
Pilot Sven (German Sky Group) konnte die Martinair-Cargo-Bid **MPH62** (SKBO → TJBQ, MD11/PH-MCU, 78.1 t Cargo) nicht starten. ASA-ACARS blockierte mit `aircraft_mismatch`:
...
```rust
// crates/asa-acars-mqtt/src/lib.rs:763
...
- `client/src-tauri/crates/asa-acars-mqtt/src/lib.rs` — Initial-Phase-Publish entfernt.
...
**localStorage-State:**
- `asa-acars.update.first_seen.{version}` — wann das Update zuerst erkannt wurde (für Stage-Berechnung)
- `asa-acars.update.dismissed_until` — Snooze-Ablauf-Timestamp
- `asa-acars.update.last_check_at` — letzter erfolgreicher Check (für Focus-Re-Check-Throttle)
...
User-Feedback: „die beiden Berechnungen im Web und in ASA-ACARS müssen gleich sein". Audit hat starke Drift aufgedeckt — Pilot-App (`LandingPanel.tsx`) und Live-Monitor (`LandingAnalysis.tsx`) hatten zwei separate Sub-Score-Tabellen mit unterschiedlichen Schwellen, Bands, Coach-Tipps und Rollout-Metriken. Derselbe Flug bekam je nach Plattform andere Teilnoten.
...
- `asa-acars-live/webapp/src/components/LandingAnalysis.tsx` — Label-Drift, kg-Einheit, G-Schwellen
...
Adrian (ASA-Pilot) hat zwei konkrete Pain-Points gemeldet:
...
2. **PasStudio-Loadsheet wird nicht erkannt** — wenn der Pilot in PasStudio neu plant und sich der Block-Fuel ändert, hält ASA-ACARS noch den alten OFP. Der Refresh-Button existiert zwar im ActiveFlight-Header, war aber nicht prominent genug.
...
User-Reports DLH 1731, CFG 9746 LDZA→EDDM (MSFS Fenix) sowie ASA 302 X-Plane DA40 Bush-Strip — drei Probleme im Anflug-/Touchdown-Bereich:
...
Bisher waren die v0.5.39+ TouchdownWindow-Forensik-Felder (`vs_at_edge`, Multi-Window-VS, Peak-G post-TD, Flare-Quality-Score) nur in der asa-acars-live Webapp sichtbar. Pilot musste nach dem Flug ins Webportal wechseln um den Volanta-/DLHv-Vergleich zu sehen.
...
- gleiches im asa-acars-live `importer.ts` für FSM-Replay von pre-v0.5.40 historische Logs
...
| Volanta | 66 fpm |
| DLHv-Tool | 62 fpm |
| ASA-ACARS v0.5.40 (msfs_simvar_latched) | **-101 fpm** ❌ |
| ASA-ACARS v0.5.41 `vs_at_edge` | **-66 fpm** ✅ exakt Volanta |
...
User-Vergleich vom DLH-1331-Flug (GMMN→EDDF, Fenix A321): ASA-ACARS meldete -205 fpm / 0.99G, Volanta -87 fpm / 1.14G, DLHv-Tool -96 fpm / 1.18G. Root-Cause-Analyse zeigte: ein 1.86-s-Loch im JSONL-Position-Stream genau im Touchdown-Moment, weil der Streamer-Tick im selben Loop phpVMS-POSTs ausführt (200-1500 ms HTTP-Latenz) und die adaptive 500-ms-Cadence stretcht. ASA-ACARS griff daher auf MSFS's instantaneous `TOUCHDOWN_VELOCITY` SimVar zurück, während Volanta/DLHv smoothed VS-Mittel über ~500-1000 ms verwenden — physikalisch repräsentativer für das was der Pilot fühlt.
...
- `client/src-tauri/crates/asa-acars-mqtt/src/lib.rs`: +57 Zeilen (TouchdownPayload-Erweiterung)
- asa-acars-live: webapp `LandingAnalysis.tsx` neue Card + recorder `importer.ts` landing_analysis-Backfill
...
User-Report aus dem ASA-301 GA-Flug: Pilot hatte instabilen Anflug (Bank ±7° unter 200 ft AGL, V/S -625 fpm bei 330 ft AGL), hätte durchstarten sollen, hat aber durchgezogen → -900 fpm Hard Landing. ASA-ACARS hat das **nicht** in real-time geflagged — Pilot bekam keine Warnung dass die Approach-Kriterien verletzt wurden.
...
**Fix:**
- Beim Ersten-Run nach Auto-Detection: erkannte Sprache EXPLIZIT in `localStorage["asa-acars.lang"]` schreiben
...
User-Report aus dem ASA-301 GA-Flug (Cessna 152 in X-Plane 12, Forensik-Log analysiert): peak_vs_fpm=-33 fpm gemeldet, peak_g_force=1.36 — passte nicht zueinander. Die echte TD-V/S war vermutlich -300 bis -400 fpm.
...
**Problem:** ASA-ACARS hat bisher SimBrief-OFP für jeden Bid verlangt (siehe `lib.rs` Z.4848: `"no aircraft on this bid — please prepare a SimBrief OFP first"`). Für VFR-Flüge unterstützt SimBrief aber kein OFP-Routing — Pilot konnte zwar Bid in phpVMS erstellen, ASA-ACARS verweigerte aber den Start.
...
1. **Aircraft-Picker** mit Suche
   ...
   - Sim-Default-Auswahl: ASA-ACARS sieht den im Sim geladenen Aircraft → vorausgewählt mit Match-Erkennung über Registration ODER ICAO
...
- DB-Backup pre-deploy: `asa-acars-live.db.backup-pre-landing-quality`.
...
Höhenfilter über `MSL_altitude − arr_airport_elevation` statt `AGL`. ASA-ACARS-Client sucht arr-Airport-Elevation aus dem phpVMS-API-Cache (state.airports.elevation). Fallback auf AGL wenn unbekannt — `approach_used_hat`-Flag in PIREP zeigt welche Methode genutzt wurde.
...
- **MQTT-Payload (`asa-acars-mqtt`)**: TouchdownPayload um 5 Felder erweitert (`approach_vs_jerk_fpm`, `approach_ias_stddev_kt`, `approach_excessive_sink`, `approach_stable_config`, `approach_used_hat`).
...
- DB-Backup pre-deploy: `asa-acars-live.db.backup-pre-approach-stability-v2`.
...
MSFS-SimConnect hat eine inverse Konvention: `PLANE PITCH DEGREES` reportet **positive Werte wenn die Nase UNTER dem Horizont** ist (= Universal-Aviation-Konvention macht es umgekehrt: positiv = nose-up). ASA-ACARS las den Wert ohne Sign-Flip und schrieb daher invertierte Werte in alle MSFS-Pilot-PIREPs:
...
Pro Position-Tick schickt der Client jetzt `client_version: "0.5.24"`. Der asa-acars-live-Monitor sieht damit pro Pilot welche Build-Version sendet — nützlich für:
...
🎯 **Forensik-Werkzeuge: asa-acars-live-Monitor sieht jetzt alles was der Client sieht.** Plus harte Fixes für Session-Splitting bei Hin/Rück-Flügen und leere ICAO-Felder.

### ✨ Neu

**1. Auto-Upload des kompletten Flight-Logs nach PIREP-File**

Nach erfolgreichem `file_pirep` lädt der Client das komplette JSONL-Logfile automatisch als gzip an asa-acars-live (`POST /api/flight-logs/upload`). Der VA-Owner kann es dann über den **„📥 Client-Log"-Button** in der History-Detail-View herunterladen — ohne den Piloten kontaktieren zu müssen.
...
**2. Touchdown-Algorithmen-Forensik im asa-acars-live-Monitor**

Bei jedem Touchdown laufen MSFS-Time-Tier- und X-Plane-Lua-30-Sample-Schätzer schon parallel — jetzt kriegt asa-acars-live alle Zwischenergebnisse und kann **Algorithmen-Disagreements** (= |xp_estimate − msfs_estimate| > 50 fpm) sichtbar machen für FSM-Edge-Case-Analyse.
...
### 📊 Server-Side (asa-acars-live, parallel deployed)
...
- Backward-kompatibel: alte Server (ohne neue Forensik-Felder) ignorieren die neuen Optional-Payload-Felder. Neue Server (= asa-acars-live ab heute) extrahieren typed.
...
3. **MQTT Live-Tracking** zur asa-acars-live VPS — komplett unsichtbar im Hintergrund
...
### 📡 Teil 3: MQTT Live-Tracking zur asa-acars-live VPS

**NEUE Crate** `client/src-tauri/crates/asa-acars-mqtt/` integriert. Komplett unsichtbares Hintergrund-Feature (KEINE UI, KEIN Settings-Tab, KEIN Toggle).
...
- Provision-Failure ist non-fatal — ASA-ACARS funktioniert exakt wie ohne MQTT
- LWT (last-will-and-testament) sorgt dafür dass beim Crash der OFFLINE-Status kommt
- Topic-ACL: jeder Pilot kann nur in `asa-acars/<va>/<seine-id>/#` publishen
...
1. ASA-ACARS-Update auf v0.5.3 installieren (auto-update)
...
- Sendet einen einmaligen JSON-„touchdown"-Paket über UDP an die ASA-ACARS-App auf `127.0.0.1:49001`.
...
### 🆕 Auto-Install im ASA-ACARS-Client
...
- **„Plugin installieren"-Button** lädt die zur installierten Client-Version passende Plugin-Zip von GitHub und entpackt nach `<X-Plane>/Resources/plugins/ASA-ACARS/`
...
- GitHub-Actions-Pipeline erweitert: Plugin-Build-Matrix (Win/Mac/Linux) + Plugin-Package-Job, der die drei `.xpl` zu `ASA-ACARS-XPlane-Plugin-vX.Y.Z.zip` zusammenfasst und ans Release uploaded
...
### 🐛 Behoben (X-Plane only)
- **Landing-Rate / peak_vs_fpm war bei X-Plane immer ~0** auch bei klar härteren Landungen. Pilot-Log heute (EWL6822 LEPA→EDDG, A320, sichtbare Sinkrate -350 fpm beim Aufsetzen): ASA-ACARS scorete „smooth, peak_vs_fpm: +5.7" — Touchdown-Window enthielt nur Post-Rollout-Daten.
...
Vier Lifecycle-Events werden jetzt automatisch in den ASA-Discord-Channel gepostet, im Stil etablierter VA-Bots:
...
Layout angelehnt an den ASA-Bot-Stil:
...
Webhook-URL ist hardcoded für ASA (`#flights`-Channel). Posts laufen fire-and-forget (`tokio::spawn`) — Discord-Latenz blockt nie den Flugverlauf.
...
### Behoben
- **MSL-Höhe weicht im Cruise um ~1.000 ft ab.** Wir lasen `sim/flightmodel/position/elevation` (= TRUE MSL, geographische Höhe über Sea Level), das aber bei nicht-ISA-Atmosphäre vom Indicated-Altitude abweicht. Pilot Michel D. sah heute auf YBBN→NWWW bei FL390 / OAT −46 °C → ASA-ACARS meldete 40.009 ft, PFD korrekt 39.000 ft (Differenz exakt die ISA-Deviation × 4 ft/°C). Im Sinkflug konvergierten die Werte wieder. Jetzt: `sim/cockpit2/gauges/indicators/altitude_ft_pilot` — Indicated, exakt was der Pilot sieht.
...
Konsolidierter 0.3.x-Release. Bündelt das komplette SimBrief-Integration-Paket (Phase H.7), erweiterte X-Plane-Telemetrie, Live-Block-Fuel-Fix, das Loadsheet-Feature, OFP-Mismatch-Detection, UX-Polish nach dem ASA-Live-Test sowie das **neue Divert-Manual-PIREP-Routing**.
...
- **Auto-Reconnect hart abgesichert.** Re-Subscribe-Loop alle 5 s wenn State ≠ Connected. Funktioniert in allen Szenarien: ASA-ACARS startet vor X-Plane, X-Plane Restart, X-Plane Crash, Aircraft-Wechsel mit Daten-Stillstand.
