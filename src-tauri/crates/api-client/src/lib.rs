...
pub struct Profile {
    ...
    /// `/api/user` returns it; only ACTIVE pilots may use ASA-ACARS
...
/// (UUID setups). Live bug from a ASA pilot 2026-05-03: `/api/user/bids`
...
pub struct SimBriefOfp {
    ...
    /// cruise reference so ref-less flights (ASA `flights.level` is almost
...
pub struct FileBody {
    ...
    ///
    /// Consequence before this was sent: every ASA-ACARS PIREP on ASA had a
...
pub mod pirep_source {
    ...
    // it was trying to avoid. Verified against phpvms@dev and the live ASA
...
impl Client {
    ...
    /// GsgLogbook-Modul (ASA phpVMS): liest das Pilot-Logbuch LIVE über die
    ...
    /// stehen in `phpvmsacars` und werden von der ASA-API mit ausgeliefert,
    /// zusammen mit IAS, Steig-/Sinkrate und Treibstoff.
    ///
    /// Auth: phpVMS-`api_key`. Die ASA-Routen haengen an phpVMS' eigener
    ...
    /// Gemessen am ASA-Live-Server (21.08.2026, 377 POSTs eines Fluges):
...
mod tests {
    ...
    fn rejects_http_to_routable_host() {
        ...
        Connection::new("https://atlanticstarairways.com", "k").unwrap();
    }
    ...
    fn paxstudio_claim_endpoint_builds_correct_url() {
        let conn = Connection::new("https://atlanticstarairways.com", "k").unwrap();
        let client = Client::new(conn).unwrap();
        let path = format!("/api/paxstudio/ofp/{}/claim", "1777622821_5F3E3B3842");
        let url = client.endpoint(&path).unwrap();
        assert_eq!(
            url.as_str(),
            "https://atlanticstarairways.com/api/paxstudio/ofp/1777622821_5F3E3B3842/claim"
        );
    ...
    /// Reproduces the live bug from a ASA pilot 2026-05-03: the
    ...
    fn prefile_body_flight_id_serialization() {
        let mut body = PrefileBody {
            ...
            source_name: "ASA-ACARS/test".into(),
            ..Default::default()
        };
