...
/// Discord Application ID — hardcoded, sem dependência de endpoint externo.
const DISCORD_APP_ID: &str = "1518011442589077604";

/// Einmaliger Init im Tauri-setup-Hook. Spawnt den Manager + setzt a App-ID
/// fixa + ruft enable quando o Pilot em Run anterior já tinha aceite.
///
...
pub fn init(app: &AppHandle) {
    ...
    tauri::async_runtime::spawn(async move {
        // App-ID fixo — direto, sem fetch ao servidor.
        refresh_app_id(&manager).await;
...
/// Define a Discord App-ID fixa no Manager.
/// Idempotente — sem chamadas de rede, sem pontos de falha externos.
async fn refresh_app_id(manager: &Arc<DiscordPresenceManager>) {
    manager.set_app_id(DISCORD_APP_ID.to_string()).await;
}
...
pub async fn discord_rpc_set_settings(
    ...
    // Garantir que a App-ID fixa está sempre definida no manager.
    refresh_app_id(&m).await;
