...
pub struct LandingRecord {
    ...
    /// `runway_geometry_trust_check` in `asa_acars_app::lib`.
    #[serde(default)]
...
mod pending_bid_cleanup_queue_tests {
    ...
    fn temp_queue() -> (PendingBidCleanupQueue, PathBuf) {
        ...
        let dir = std::env::temp_dir().join(format!(
            "asa-acars-pendingbidqueue-test-{}-{n}",
            std::process::id()
        ));
    ...
    fn v170_datensatz_ueberlebt_hin_und_zurueck() {
        let mut r: LandingRecord = serde_json::from_str(
            r#"{"pirep_id":"x","touchdown_at":"2026-08-11T17:34:42Z",
                ...
                "airline_icao":"ASA","dpt_airport":"EDDH","arr_airport":"EDDF",
