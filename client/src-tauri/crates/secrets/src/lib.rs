//! File-based secrets storage for ASA-ACARS (phpVMS API key, MQTT creds).
//!
...
//!
//! Mac users hit a wall of "ASA-ACARS möchte auf den Schlüsselbund
...
//! likely have process-injection access to the running ASA-ACARS
...
mod tests {
    ...
    fn setup() {
        INIT.call_once(|| {
            let dir = std::env::temp_dir().join("asa-acars-secrets-test");
