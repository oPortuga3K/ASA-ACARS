...
/// ASA-Server. Insbesondere: "T" ist TECHNICAL_TEST, nicht Training (echtes
...
function AirlineLogoView({
  ...
  const showFallback = !logo || error;

  return (
    <div className={`${classNameBase} ${showFallback ? `${classNameBase}--placeholder` : ""}`}>
      {!showFallback ? (
        <img src={logo as string} alt={altText} onError={() => setError(true)} />
      ) : (
...
export function BidsList({
   ...
   *  die ASA-Seite ihre Daten live holt → Browser-Tab kann bis 30 s laden. */
  ...
  async function openOfp(flight: Flight) {
    ...
    // paxstudio-theme convention; works on ASA and most paxstudio-based VAs.
      ...
      {state.kind === "ready" && (() => {
        ...
        return (
          <>
              ...
              return (
                <MainBidCard
                  ...
                  onWeather={() => {
                    ...
                    void openUrl("https://atlanticstarairways.com/weatherbriefing").catch(() => {});
                  }}
                />
              );
            })()}
            ...
            {restBids.length > 0 && (
              <ul className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mx-8 mt-6">
                {restBids.map((bid) => {
                  const f = bid.flight;
                  ...
                  return (
                    <li key={bid.id}>
                      <button
                        type="button"
                        className="w-full text-left bg-zinc-900 border border-zinc-800 rounded-2xl p-4 flex flex-col hover:border-zinc-700 hover:bg-zinc-800/50 transition-all group"
                      ...
                      >
                        <div className="flex justify-between items-center mb-4">
                           <div className="flex items-center gap-3">
                             <AirlineLogoView
                               logo={airlineLogo}
                               fallbackText={f.airline?.icao ?? monogram}
                               altText={f.airline?.name ?? icaoCallsign ?? ""}
                               classNameBase="w-8 h-8 rounded-lg bg-zinc-950 flex items-center justify-center border border-zinc-800 text-xs font-bold text-zinc-400"
                             />
                             <span className="font-bold text-white tracking-wide">{icaoCallsign}</span>
                           </div>
                           {!f.simbrief?.id && (
                             <span className="bg-amber-500/10 text-amber-500 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">{t("bid_card.no_ofp_badge")}</span>
                           )}
                        </div>
                        
                        <div className="flex items-center justify-between">
                           <div className="flex flex-col">
                             <span className="text-xl font-bold text-white">{dpt}</span>
                             <span className="text-sm font-mono text-zinc-500">{dptTimeDisplay ? `${dptTimeDisplay}z` : "--:--"}</span>
                           </div>
                           
                           <div className="flex-1 px-4 flex flex-col items-center">
                              <svg className="w-5 h-5 text-zinc-600 rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
                              </svg>
                           </div>

                           <div className="flex flex-col text-right">
                             <span className="text-xl font-bold text-white">{arr}</span>
                             <span className="text-sm font-mono text-zinc-500">{arrTimeDisplay ? `${arrTimeDisplay}z` : "--:--"}</span>
                           </div>
                        </div>

                        <div className="mt-4 pt-3 border-t border-zinc-800/50 flex flex-wrap gap-2 text-xs font-medium text-zinc-400">
                          {[aircraftType, formatDistanceNm(f.distance?.nmi ?? null, i18n.language), formatFlightTime(f.flight_time, i18n.language)]
                            .filter(Boolean)
                            .map((item, i) => (
                              <span key={i} className="bg-zinc-950/50 px-2 py-1 rounded-md border border-zinc-800/50">{item}</span>
                            ))}
                        </div>
                      </button>
                    </li>
...
function MainBidCard({
  ...
  return (
    <article className="mx-8 bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-xl mb-6 relative">
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-sky-400 to-blue-600"></div>

      <div className="p-8">
        {/* Top bar with Airline logo and Meta */}
        <div className="flex justify-between items-start mb-8">
          <div className="flex items-center gap-4">
            <AirlineLogoView
              logo={airlineLogo}
              fallbackText={flight.airline?.icao ?? airlineMonogram(flight)}
              altText={airlineName ?? icaoCallsign ?? ""}
              classNameBase="w-12 h-12 rounded-xl bg-zinc-950 flex items-center justify-center border border-zinc-800 shadow-inner font-bold text-lg text-white"
            />
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                {isNextFlight && (
                  <span className="bg-sky-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">{t("bid_card.next_badge")}</span>
                )}
                <span className="text-xl font-bold text-white tracking-wide">{icaoCallsign}</span>
              </div>
          ...
          <div className="flex gap-4 text-sm font-medium">
            {flight.flight_type && <span className="bg-zinc-800 text-zinc-300 px-3 py-1 rounded-full border border-zinc-700">{flightTypeLabel(flight.flight_type)}</span>}
            <span className="bg-zinc-800 text-zinc-300 px-3 py-1 rounded-full border border-zinc-700">{formatDistanceNm(flight.distance?.nmi ?? null, i18n.language)}</span>
            <span className="bg-zinc-800 text-zinc-300 px-3 py-1 rounded-full border border-zinc-700">{formatFlightTime(flight.flight_time, i18n.language)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-8 relative">
          <div className="flex flex-col w-1/4">
            <span className="text-4xl font-bold text-white mb-2">{dpt}</span>
            <div className="flex flex-col">
              <span className="text-xl font-mono text-zinc-300">{dptTimeDisplay ? `${dptTimeDisplay}z` : "--:--"}</span>
              {dptName && <span className="text-sm text-zinc-500 truncate mt-1">{dptName}</span>}
              {dptCountdown && (
                <span className={`text-xs mt-1 font-bold ${dptCountdown.overdue ? "text-amber-500" : "text-emerald-400"}`}>
          ...
          <div className="flex-1 px-8 flex flex-col items-center">
             <div className="w-full flex items-center gap-4 relative">
               ...
               <svg className="w-8 h-8 text-sky-400 rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" />
               </svg>
             ...
             <div className="mt-4 flex flex-col items-center text-center gap-1">
               {(aircraftType || registration) && (
                 <span className="bg-sky-900/30 text-sky-400 px-3 py-1 rounded-full text-xs font-bold font-mono tracking-wider">
               ...
               {!ofpMismatch && (plan?.alternate || cruiseLevel) && (
                 <div className="mt-2 text-xs font-mono text-zinc-500">
                   {plan?.alternate && (
                     <>
                       <span className="uppercase tracking-widest">{t("bid_card.altn_label")}</span>{" "}
                       <span className="text-zinc-300 font-bold">{plan.alternate}</span>
                     </>
                   )}
          ...
          <div className="flex flex-col w-1/4 text-right">
            <span className="text-4xl font-bold text-white mb-2">{arr}</span>
            <div className="flex flex-col items-end">
              <span className="text-xl font-mono text-zinc-300">{arrTimeDisplay ? `${arrTimeDisplay}z` : "--:--"}</span>
              {arrName && <span className="text-sm text-zinc-500 truncate mt-1">{arrName}</span>}
            </div>
          </div>
        </div>

        {/* Rubrics (Fuel, Weights, Load) */}
        {planLoading ? (
        ...
        {/* Mismatch & Error Banners */}
        {!hasSimBriefId && (
          <div className="bg-amber-500/10 border border-amber-500/20 text-amber-500 p-4 rounded-xl mb-6">
            <strong className="block mb-1">{t("bids.no_ofp_title")}</strong>
            <span className="text-sm">{t("bids.no_ofp_hint")}</span>
          </div>
        )}

        {ofpMismatch && plan && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl mb-6">
            <strong className="block mb-3">{t("bids.ofp_mismatch_title")}</strong>
            <div className="grid grid-cols-2 gap-4 mb-3 font-mono text-sm bg-black/20 p-3 rounded-lg">
              <div>
                <span className="block text-xs uppercase tracking-widest text-red-400/70 mb-1">{t("bids.ofp_mismatch_bid")}</span>
                <span>
              ...
              <div>
                <span className="block text-xs uppercase tracking-widest text-red-400/70 mb-1">{t("bids.ofp_mismatch_ofp")}</span>
                <span>
            ...
            <span className="text-sm">{t("bids.ofp_mismatch_hint")}</span>
          </div>
        )}

        {!ofpMismatch && hasSimBriefId && (planLoading || (planError && !plan)) && (
          <div className="mb-6 text-sm font-medium">
            {planLoading && <span className="text-zinc-400 animate-pulse">{t("bids.simbrief_plan")} …</span>}
            {planError && !plan && <span className="text-red-400">{planError}</span>}
          </div>
        )}

        {hasRoute && (
          <div className="bg-black/20 rounded-xl p-4 mb-6 font-mono text-sm text-zinc-400 border border-zinc-800/50">
            <span className="text-zinc-500 font-bold uppercase tracking-widest mr-3">{t("bids.route")}</span>
      ...
      <div className="bg-zinc-950/50 p-6 border-t border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="bg-zinc-900 rounded-lg p-1 border border-zinc-800 flex items-center">
            <button
              type="button"
              className={`px-4 py-2 rounded-md font-bold text-sm transition-colors ${mode === "ifr" ? "bg-sky-500 text-white" : "text-zinc-500 hover:text-white"}`}
              onClick={() => setMode("ifr")}
            ...
            <button
              type="button"
              className={`px-4 py-2 rounded-md font-bold text-sm transition-colors ${mode === "vfr" ? "bg-sky-500 text-white" : "text-zinc-500 hover:text-white"}`}
              onClick={() => setMode("vfr")}
            >
              {t("bid_card.mode_vfr")}
            </button>
          </div>
          <span className="text-sm text-zinc-500 font-medium">{primaryHint}</span>
        </div>

        <div className="flex items-center gap-4">
          <div className="flex gap-2">
            {hasSimBriefId && (
              <button type="button" className="px-4 py-2 text-sm font-semibold text-zinc-400 hover:text-white transition-colors" onClick={onOfp}>
                {t("bids.open_ofp")}
              </button>
            )}
            <button type="button" className="px-4 py-2 text-sm font-semibold text-zinc-400 hover:text-white transition-colors" onClick={onFlightPage}>
              {t("bids.open_flight_page")}
            </button>
            <button type="button" className="px-4 py-2 text-sm font-semibold text-zinc-400 hover:text-white transition-colors" onClick={onWeather}>
          ...
          <button
            type="button"
            className="px-8 py-3 bg-sky-500 hover:bg-sky-400 text-white font-bold rounded-xl shadow-lg shadow-sky-500/20 transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100"
      ...
      {distanceToDptNm !== null && (
        <div className={`absolute top-8 right-8 px-4 py-2 rounded-full font-bold text-xs uppercase tracking-wider ${tooFar ? "bg-red-500/10 text-red-400 border border-red-500/20" : "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"}`}>
          {distanceToDptNm < 1
      ...
      {startError && (
        <div className="mx-8 mb-6 bg-red-500/10 text-red-400 border border-red-500/20 p-4 rounded-xl text-sm font-bold">
          {startError}
        </div>
      )}
      {startWarning && (
        <div className="mx-8 mb-6 bg-amber-500/10 border border-amber-500/20 p-4 rounded-xl">
          <div className="text-amber-500 font-bold mb-2">{t("manual_flight.warning_title")}</div>
          <div className="text-amber-400/80 text-sm mb-4">{startWarning}</div>
          <button
            type="button"
  ...
  return (
    <div className="bg-zinc-950/50 rounded-xl border border-zinc-800/50 p-4">
      <div className="flex justify-between items-center mb-3">
        <span className="text-sm font-bold text-zinc-500 uppercase tracking-widest">{title}</span>
        {unit && <span className="text-xs font-mono text-zinc-600">{unit}</span>}
      </div>
      {headline != null && (
        <div className={`flex items-baseline flex-wrap gap-2 mb-4 ${insetDanger ? "text-red-400" : "text-white"}`}>
          <span className="text-3xl font-bold font-mono">{headline}</span>
          <span className="text-sm font-bold text-zinc-500 uppercase tracking-wider">{headlineLabel}</span>
          {inset && <span className="text-xs font-bold text-sky-400 ml-auto bg-sky-900/20 px-2 py-1 rounded-md">{inset}</span>}
        </div>
      )}
      {rows.length > 0 && (
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-sm">
          {rows.map((row, i) => {
             ...
             return (
              <div key={i} className="flex justify-between items-center border-b border-zinc-800/50 pb-1">
...
function AutoStartSkipBanner() {
  ...
  return (
    <div
      className="mx-8 mt-6 bg-amber-500/10 border border-amber-500/20 text-amber-500 p-4 rounded-xl flex items-center gap-3"
      role="status"
      aria-live="polite"
    >
      ...
      <div>
        <strong className="mr-2 font-bold">{t("bids.auto_start_skip.title")}</strong>
